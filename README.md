<div align="center">

<img src="https://github.com/SP-XD/SP-XD/blob/main/images/hellocoders_rounded.gif?raw=true" width="60%" />
<br>
<img src="https://github.com/SP-XD/SP-XD/blob/main/images/dev-working_rounded.gif?raw=true" width="40%" />

---

## 👋 Hello, I'm **Taleb Ahmed**
🎓 **Licence Student in Computer Networks & Telecommunications**  
🌍 Passionate about **Networking, Cloud, Linux & Programming**

---

<details>
<summary><b>🎧 Busy coding & vibing to music</b></summary>

[![Spotify](https://spotify-readme.sp-xd.vercel.app/api/spotify)](https://open.spotify.com)

</details>

---

![Profile Views](https://komarev.com/ghpvc/?username=TalebAhmed&style=flat&color=orange)
[![GitHub Followers](https://img.shields.io/github/followers/TalebAhmed?style=flat)](https://github.com/TalebAhmed)

📬 **Telegram**: [@TalebAhmed](https://t.me/)  

</div>

---

## 🚀 Tools & Technologies

![Networking](https://img.shields.io/badge/Networking-0078D4?style=flat)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Cloud](https://img.shields.io/badge/Cloud-4285F4?style=flat&logo=googlecloud&logoColor=white)

### 💻 Programming & Scripting
![Python](https://img.shields.io/badge/Python-FFD43B?style=flat&logo=python&logoColor=darkgreen)
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=java)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=flat&logo=javascript)

### 🌐 Web
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3)

### 🗄️ Databases & Tools
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql)
![SQLite](https://img.shields.io/badge/SQLite-07405E?style=flat&logo=sqlite)
![Git](https://img.shields.io/badge/Git-E44C30?style=flat&logo=git)
![VSCode](https://img.shields.io/badge/VS_Code-0078D4?style=flat&logo=visualstudiocode)

---

## 🧠 About Me (Code Style)

```dart
class TalebAhmed {
  final field = "Computer Networks & Telecommunications";

  final skills = {
    "Networking": ["Routing", "Switching", "IP", "LAN/WAN"],
    "Systems": ["GNU/Linux", "Windows Server"],
    "Programming": ["Python", "C", "C++", "Java"],
    "Web": ["HTML", "CSS", "JavaScript"],
    "Databases": ["MySQL", "SQLite"],
    "Tools": ["Git", "Wireshark", "VS Code"]
  };

  final interests = [
    "Cloud Computing ☁️",
    "Network Security 🔐",
    "System Administration 🖥️"
  ];
}
